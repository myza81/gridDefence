"""Tests for Equipment Registry's permission-catalog registration
(app/modules/equipment_registry/bootstrap.py) — iam-module.md §7.3.
"""
...
from app.modules.equipment_registry.bootstrap import run_bootstrap
...
def test_registers_read_and_write_permissions(db_session: Session) -> None:
    ...
    assert "equipment_registry.read" in permission_ids
    assert "equipment_registry.write" in permission_ids
...
def test_is_idempotent(db_session: Session) -> None:
    ...
    service = IAMService(db_session)
    permission_ids = [p.permission_id for p in service.list_permissions()]
    assert permission_ids.count("equipment_registry.write") == 1
...
def test_administrator_receives_both_permissions(db_session: Session) -> None:
    ...
    assert "equipment_registry.read" in admin_role.permissions
    assert "equipment_registry.write" in admin_role.permissions
...
def test_engineer_receives_write_but_not_read(db_session: Session) -> None:
    ...
    assert "equipment_registry.write" in grants
    assert "equipment_registry.read" not in grants
...
def test_viewer_receives_read_but_not_write(db_session: Session) -> None:
    ...
    assert "equipment_registry.read" in grants
    assert "equipment_registry.write" not in grants
