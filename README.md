...
**Deployment gaps of this same shape have recurred more than once** — code was correct and fully tested, but the persistent dev database was never re-synchronized with it:
...
- The Automatic Load Shedding Functionality Registry's "Add Functionality Record" button and empty-state create action appeared to be missing entirely in the browser, for every user including Administrator. The frontend code was correct (`FunctionalityListPage.tsx` correctly gates the button on `permissions.has("automatic_load_shedding_functionality.write")`); the module's own `bootstrap.py` had simply never been run against the persistent dev database when the module shipped, and — worse — this list itself was never updated to include it, so there was no documented reminder to do so. `permission` had zero rows for `automatic_load_shedding_functionality.*`, and consequently no role, including Administrator, had the permission the UI correctly requires. Root-caused by querying the dev database directly (`SELECT * FROM permission WHERE permission_id LIKE 'automatic_load_shedding%'` returned zero rows) rather than assuming a frontend defect. Fixed by running `python -m app.modules.automatic_load_shedding_functionality.bootstrap` and adding it to this list. While investigating, `network_model.bootstrap` was found to have the same never-run gap (its `network_model.read` permission was also absent) — harmless today only because no `network_model` endpoint currently gates on that permission, but left as a standing landmine; also added to this list.
