...
def fetch_asset(name: str, spec: dict, base_dir: Path, force: bool) -> bool:
    ...
    if not url:
        print(f"[{name}] no URL configured in manifest - skipping.")
        print(f"        Set assets.{name}.url to an internal/licensed source (see README.md).")
        return False
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if expected:
            ...
            print(f"[{name}] no sha256 in manifest - skipping integrity check ({human(tmp_file.stat().st_size)})")
