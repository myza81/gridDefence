...
> **Status update (Engineering Scheme Architecture Pack, six-workshop conclusions).** Phases 6–8 (UFLS/UVLS/EMLS) below describe a six-state `Draft → Under Review → Approved → Active` workflow and, for Phase 6, an ALSF-absence hard-selection-block that are both **superseded**. See [scheme-roadmap-corrections.md](scheme-roadmap-corrections.md) for the full, itemized correction list (lifecycle wording, the ALSF finding-not-block correction, the Cross-Scheme-Compliance-stub-to-unified-findings correction) and Phase 5's own Network Model deliverable list (superseded per [network-model-module.md](network-model-module.md)'s own status note and [ADR-017](../adr/ADR-017-boundary-pocket-assignment-architecture.md)). This document's original text below is preserved as the historical planning record, per CLAUDE.md §5.2, and is not rewritten.
...
**STATUS: Complete.** Implemented as the first concrete consumer of the Shared Defence-Scheme Platform (Shared Platform Sprints 1-6), per [ufls-architecture.md](ufls-architecture.md) — not per this section's own original text below, which describes the six-state `Draft → Under Review → Approved → Active → Superseded → Archived` lifecycle and a `UflsLoadBlock`/MW-capture-at-Approval design, both **superseded** by [ADR-015](../adr/ADR-015-defence-scheme-version-lifecycle-simplification.md) and [shared-defence-scheme-domain-model.md](shared-defence-scheme-domain-model.md) (see [scheme-roadmap-corrections.md](scheme-roadmap-corrections.md)). As built: `UflsScheme`/`UflsSchemeVersion` (composing `scheme_platform`'s `SchemeVersionMixin` and `SchemeVersionLifecycleService` for the ratified four-state `Draft → Published → Superseded | Entered in Error` lifecycle) / `UflsStage` (referencing a Stage Setting Registry `StageSetting`, external-study `target_mw` only, never computed) / `UflsDirectAssignment` and `UflsPocketAssignment`+`UflsPocketAssignmentOpeningPoint` (attached directly to a stage — no load-block grouping layer) / `UflsAuditLog`. Relay Capability Verification and Sensitive Customer Review are real calls (ALSF and Sensitive Customer Registry were both already complete); Cross-Scheme Compliance is not yet integrated (Phase 10 does not exist yet) and is not called at all, rather than stubbed, since no interface contract for it exists to stub against faithfully. Full detail: [ufls-architecture.md](ufls-architecture.md). The plan below is preserved as the historical record (CLAUDE.md §5.2) and is not rewritten.
...
| **Dashboard module has no dedicated architecture document** | **Recommend resolving before Phase 11**, not before Phase 0 | A lightweight design note (not necessarily a full A8 document) scoping Phase 11 more precisely than "compose whatever already exists" would reduce risk at that specific phase; does not block Phases 0–10. [regional-engineering-analytics-architecture.md](regional-engineering-analytics-architecture.md) §7 already resolves one concrete piece of this scoping — Dashboard composes the Continuous Evaluation Engine's regional-analytics service read-only, never recomputing it — and should be referenced directly when Phase 11's own design note is written |
