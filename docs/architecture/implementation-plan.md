...
**Dependencies:** Phase 6 (template + reusable frontend components), Phase 5, Phase 4, Phase 3.6 (Automatic Load Shedding Functionality Registry — real, not stubbed), Phase 2, Phase 1. Sensitive Customer Registry (stubbed if not yet built, same as Phase 6).
...
**Dependencies:** Phase 6, Phase 5, Phase 4, Phase 2, Phase 1. **Deliberately not** Phase 3.6 (Automatic Load Shedding Functionality Registry) — see the hard exclusion above. Sensitive Customer Registry (stubbed if not yet built, same as Phase 6/7).
...
| **Sensitive Customer Registry has no architecture document of any kind** — no EDR, no ADR, no module document; only conceptual mentions in `domain-model.md`/`docs/engineering/glossary.md` | **Recommend resolving before Phase 6**, not before Phase 0 | Per the accepted architecture review following Phase 3.6's completion, this is the **recommended next architecture-discovery phase** — an EDR scoping the engineering question (mirroring [EDR-003](../engineering/edr/EDR-003-relay-registry-scope.md)'s discipline) followed by a module document, before any implementation (CLAUDE.md §24). Does not block Phases 0–5 or Phase 3.6, none of which consult it. Phase 6 (UFLS)'s Sensitive Customer Review integration can proceed with a stub-then-real client (see Phase 6's Backend deliverables) if this phase-numbering-free architecture-discovery work is not yet complete when Phase 6 begins — but building it first avoids that stub entirely, and avoids repeating the exact planning gap that left Relay Capability unaddressed in Phase 3's original scope (see Phase 3's status note and Phase 3.6 above). |

**Conclusion:** implementation may begin at Phase 0 immediately. Two items are worth addressing before their own dependent phase specifically (rather than before the plan as a whole): Dashboard's missing architecture document (recommended sometime during Phases 6–10, well ahead of Phase 11), and Sensitive Customer Registry's complete absence of architecture (recommended before Phase 6, since Phase 6 is the first phase whose own engineering workflow — Sensitive Customer Review — depends on it, per 03-system-workflow.md).
