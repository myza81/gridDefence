...
- **Single Standard basemap** driven by `VITE_MAP_STYLE_STANDARD` (a governed built-in default when unset). Satellite/Hybrid/Terrain and the style selector were intentionally removed (product simplification) — the operator never chooses a basemap; the map is simply Standard → automatic neutral fallback.
...
**Geographic context vs engineering truth:** the basemap (OpenFreeMap/OSM) is *External Geographic Context*, not engineering truth. The map draws **only substation point markers** (the Registry layer) — **no connectivity/circuit/transformer geometry is drawn over the basemap**, by design: GridDefence coordinates may not align with external mapping, and drawing both would wrongly imply reconciliation. Markers ✓, connectivity ✗ by default; an externally mapped OSM "substation" is never implied to equal a GridDefence substation. Future engineering layers would be additive, opt-in and governed — never auto-merged. Full overlay philosophy + the "Standard looks sparse" root cause: [engineering-map.md](engineering-map.md) §0.3–§0.4.
