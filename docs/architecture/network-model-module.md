...
**Status update (Engineering Scheme Architecture Pack, six-workshop conclusions — Boundary Pocket).** [ADR-017](../adr/ADR-017-boundary-pocket-assignment-architecture.md) and [boundary-pocket-architecture.md](boundary-pocket-architecture.md) resolve how a Defence Scheme module constructs a Boundary Pocket assignment: as new orchestration logic built directly on **this module's own `traverse` primitive (§19.4, unmodified)**, at per-`CircuitTerminal` opening-point granularity — closing this document's own long-standing §9 rule 11 Open Question 1. **§1–§18's `CutSetDefinition`/`IslandAnalysisResult`/`ManualOverride` design remains entirely unbuilt and is not required by this decision** — it remains available as a future extension layered underneath Boundary Pocket construction (per §19.7's own "Load pocket / boundary identification" future extension entry), never built or assumed by this update. No code, schema, or interface in this document is changed by this note.

**Status update (Foundation Hardening Sprint A — IMPLEMENTED).** The above is now built. `traverse` (§19.4) gained an additive `excluded_circuit_terminal_ids` parameter — Circuit Terminal opening points, resolved independently per terminal via `EquipmentTopologyMap` (already keyed at that granularity; no PSS/E Integration change was needed) — alongside its existing whole-`Circuit` `excluded_circuit_ids`, combined additively when both are supplied. A new `evaluateBoundary` service method and `POST /network-model/boundary-pocket-evaluations` endpoint (§19.6, revised below) realize [boundary-pocket-architecture.md](boundary-pocket-architecture.md) §7's completeness evaluation directly inside this module, reusing `traverse`'s own graph-construction and BFS logic (factored into a shared `_bfs_substation_depths` helper) rather than duplicating it. The "rest of grid" reference substation ([boundary-pocket-architecture.md](boundary-pocket-architecture.md) §7's own open question) is now real configuration (`Settings.rest_of_grid_reference_substation_mnemonic`), with a per-request override. This closes this document's own §18 risk row on `EquipmentTopologyMap`'s per-terminal correlation. No Boundary Pocket entity, Scheme module, Engineering Workspace, or Evaluation Engine was implemented — this sprint built only the Network Model foundation these depend on. See §19.6, §19.8, revised below.

**Status update (Foundation Hardening Sprint A.1 — diagnostic evaluator, IMPLEMENTED).** A transient, read-only diagnostic page now consumes `evaluateBoundary` for manual engineering UAT — see [boundary-pocket-architecture.md](boundary-pocket-architecture.md)'s own status note for full detail. Not the future Pocket Builder; creates no Boundary Pocket entity, Scheme Version, or assignment. No change to `NetworkModelService`/`evaluate_boundary`'s own engineering behaviour.

**Status update ([ADR-019](../adr/ADR-019-boundary-pocket-evaluation-by-connected-component-discovery.md) — Foundation Hardening Sprint C, IMPLEMENTED, corrects Sprint A above).** `evaluateBoundary`'s mechanism is corrected from two nominated-substation `traverse()` calls to whole-graph connected-component discovery: the engineer selects only `CircuitTerminal` opening points; the Main Grid (baseline largest connected component) and every isolated island are discovered from the topology itself, never nominated. `Settings.rest_of_grid_reference_substation_mnemonic` and the per-request `rest_of_grid_substation_id`/`inside_substation_id` fields are removed entirely. `traverse` itself (§19.4), its `excluded_circuit_ids`/`excluded_circuit_terminal_ids` parameters, and `_build_bus_adjacency_map` are unchanged — the correction adds a new whole-graph component partition (`_compute_bus_components`) composed with that same unmodified adjacency primitive, never a second graph or a change to `traverse`'s own contract. See [boundary-pocket-architecture.md](boundary-pocket-architecture.md) §7 for the full corrected mechanism. §19.6, §19.8 below are updated accordingly.
...
| Tee-off `Circuit` per-terminal-subset assignment (§17) remains an open architectural question | A scheme module cannot today express "open only one leg of a three-way tee," which may understate what the model can represent for a real tee-off network configuration | Treat as an explicit open item requiring its own decision before being needed (ADR-007 Open Question 1) — do not silently assume whole-`Circuit`-only resolution is permanently sufficient. **Partially resolved for the §19 static/traversal model specifically (Foundation Hardening Sprint A):** `traverse`'s new `excluded_circuit_terminal_ids` and `evaluateBoundary`'s `circuit_terminal_ids` already express "open only one leg," each terminal resolved independently via `EquipmentTopologyMap`. This row's own §1–§18 `CutSetDefinition`-based cut-set resolution question remains open and unbuilt — the resolution above applies only to the §19 `traverse`-based path Boundary Pocket construction actually uses. |
...
- `POST /network-model/traverse` → `TraversalResult` (POST because it accepts a request body: `start_substation_id`, `excluded_circuit_ids`, `excluded_circuit_terminal_ids` [Foundation Hardening Sprint A, optional, additive to `excluded_circuit_ids`], `max_depth`, `topology_version_id` [Phase 7E, optional])
- `POST /network-model/boundary-pocket-evaluations` → `BoundaryPocketEvaluation` (corrected, [ADR-019](../adr/ADR-019-boundary-pocket-evaluation-by-connected-component-discovery.md); [boundary-pocket-architecture.md](boundary-pocket-architecture.md) §7, §10) — `circuit_terminal_ids` and an optional `topology_version_id` only; no inside/rest-of-grid substation of any kind. Returns baseline Main Grid status and every derived isolated island. No persistence — a transient, stateless evaluation, exactly like `traverse` itself.
...
**Foundation Hardening Sprint A additions (both files, above):** `excluded_circuit_terminal_ids` correctly excludes only one leg of a real three-terminal tee-off (correlated via a dedicated `operational_topology_with_correlated_tee_off_circuit` fixture) while whole-`Circuit` exclusion is shown, by contrast, to over-broadly exclude both legs; an uncorrelated or nonexistent terminal excludes nothing (mirroring `excluded_circuit_ids`'s own tolerance). This `traverse`-only test coverage is unchanged and unregressed by Sprint C, below.

**Foundation Hardening Sprint C additions ([ADR-019](../adr/ADR-019-boundary-pocket-evaluation-by-connected-component-discovery.md), superseding Sprint A's `evaluateBoundary`-specific tests listed in the prior revision of this paragraph):** baseline single-Main-Grid detection (including trivial single-Substation components tolerated as registry noise, not abnormal); no-opening-points ineffective classification; one isolated island via a tee-off leg; two isolated islands via two separate ordinary Circuits (the multi-island case the superseded two-seed mechanism could not express); parallel circuits both in service (excluding one does not isolate, excluding both does) and one already out of service (excluding the remaining in-service path alone already isolates, respecting the active snapshot); a redundant duplicate opening point on the same Circuit; an uncorrelated tee-off hub terminal, reported deterministically; a genuinely disconnected pre-existing baseline cluster, exposed as abnormal evidence; determinism across repeated calls with an identical request; no ORM session mutation after evaluation (`db_session.new`/`dirty`/`deleted` all empty); and a nonexistent-`CircuitTerminal` rejection. `RestOfGridReferenceNotConfiguredError`/`RestOfGridReferenceSubstationNotFoundError` and their tests are removed entirely, not retained — the mechanism they guarded no longer exists. Verified against both SQLite (default) and a real PostgreSQL database (`GRIDDEFENCE_TEST_DATABASE_URL`), per this project's own dual-backend testing convention.
