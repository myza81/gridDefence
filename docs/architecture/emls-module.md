...
> **Status update (Engineering Scheme Architecture Pack, six-workshop conclusions).** §7 (Domain Model) and §8 (Lifecycle/State Model) below are **superseded**, not rewritten. `EmlsSchemeVersion`'s lifecycle is now `Draft → Published → Superseded | Entered in Error` ([ADR-015](../adr/ADR-015-defence-scheme-version-lifecycle-simplification.md)). EMLS has no Stage Setting Set ([ADR-016](../adr/ADR-016-stage-setting-set-as-reusable-versioned-entity.md) §2) — `EmlsPriorityGroup` remains owned directly by the version, unaffected in shape by this update. Pocket assignment now resolves via `CircuitTerminal` selection and `traverse` ([boundary-pocket-architecture.md](boundary-pocket-architecture.md), [ADR-017](../adr/ADR-017-boundary-pocket-assignment-architecture.md)) — the engineering judgement that pocket assignment remains routine and primary-tier for EMLS specifically (§7.5) is unaffected. §7.7's Rule 1/2 participation reasoning, including the deliberate no-weakening-for-manual-invocation recommendation, is fully preserved as the shipped default under [findings-and-publication-governance-architecture.md](findings-and-publication-governance-architecture.md)'s new, more general policy mechanism ([ADR-018](../adr/ADR-018-findings-severity-and-publication-governance-separation.md)) — not weakened, only re-expressed as configurable (audited) policy rather than hardcoded logic. See [emls-engineering-philosophy.md](emls-engineering-philosophy.md) for what remains genuinely EMLS-specific, including unresolved open questions this pack does not invent answers for. §1–§6, §9 (minus lifecycle-specific rules), §10, §12–§18 remain authoritative and are **not** superseded.
