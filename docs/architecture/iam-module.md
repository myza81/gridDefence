...
> **Status update (IAM Completion Sprint — user status lifecycle and last-active-Administrator safeguard, IMPLEMENTED).** §8's own `User.status` lifecycle (`Active → Suspended → Deactivated`) is now a completed, enforced closed allow-list, not only a data-model note: `Active ⇄ Suspended`, `Active → Deactivated`, and `Suspended → Deactivated` are all live edges; `Deactivated` is terminal (no transition leads back out of it), mirroring Substation Registry's own `_STATUS_TRANSITIONS` convention exactly. Every transition requires a non-empty reason, records the acting user and timestamp, and is captured in `iam_audit_log` (§14) — see `IAMService.set_user_status` and `POST /users/{user_id}/status`. A new, permanent invariant — **at least one active Administrator must always remain** — is enforced at the service layer, inside the same transaction as the mutation, wherever an active Administrator could be lost (revoking the Administrator role, suspending, or deactivating): see `IAMService._assert_would_not_remove_last_active_administrator`. This closes exactly the gap the Development Database Recovery and Bootstrap Administrator Lifecycle investigations identified — a recovery Administrator can now be fully retired (not merely stripped of its role) once a replacement has been verified. **Explicitly out of scope, still deferred:** self-service/administrator-driven password reset (§17, unchanged), audit-history viewing, and Role retirement — none of these are implemented by this update.
...
- `User.status`: `Active → Suspended → Deactivated`. Soft-delete only (CLAUDE.md §11.6) — a `User` row is never physically deleted, since every other module's audit log may hold a permanent, immutable reference to it. **Completed (IAM Completion Sprint):** the closed allow-list of legal transitions is `Active ⇄ Suspended`, `Active → Deactivated`, and `Suspended → Deactivated` — `Deactivated` is terminal, with no transition leading back out of it, mirroring [substation-registry.md](substation-registry.md)'s own closed-allow-list convention for operational status. Every transition requires a non-empty reason and is audited (§14); no-op transitions and undefined edges are both rejected, not silently tolerated. A permanent invariant — at least one `User` must remain both `Active` and holding a currently-active grant of the `Administrator` system role — is enforced wherever that could otherwise reach zero (revoking the Administrator role, suspending, or deactivating the last such user).
...
- `users/{id}/status` — **(IAM Completion Sprint, IMPLEMENTED)** transition a user's `status` (§8); requires `iam.user.manage`, a target status, and a mandatory `change_reason`. Never accepts a username, password, or role change in the same request.
