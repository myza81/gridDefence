...
GridDefence is composed of independent bounded contexts ("modules"), each owning its own data and business rules (CLAUDE.md §6, §12). Phases 0–4 and the Automatic Load Shedding Functionality Registry (Phase 3.6, see note below the table) are complete; the remaining contexts are planned, in the dependency order [implementation-plan.md](implementation-plan.md) specifies:
...
| *(next)* Sensitive Customer Registry | Engineering Registry | Policy-exclusion classification for candidate loads | Recommended next architecture-discovery phase (unnumbered — EDR and module architecture not yet started; implementation must not begin before both are approved, per CLAUDE.md §24) |
...
**Phase numbering note (Automatic Load Shedding Functionality Registry).** The original 13-phase sequence (Phase 0 through Phase 13, [implementation-plan.md](implementation-plan.md)) never assigned this module its own phase number — it was originally sketched as an in-Equipment-Registry extension (`RelayDetail`/`RelayControlledEquipment`, `equipment-registry-module.md` §7.8), deliberately deferred out of Phase 3, then later re-scoped as its own module by [ADR-011](../adr/ADR-011-automatic-load-shedding-functionality-registry.md). It is labeled **Phase 3.6** here — an additive label only, following the same precedent already set by Phase 3.5 (Transformer Registry, also an unplanned post-Phase-3 addition to the Engineering Registry domain) — not a renumbering of any existing phase. No other phase number changes.
