...
> **Status update (Engineering Scheme Architecture Pack, six-workshop conclusions).** §7 (Domain Model) and §8 (Lifecycle/State Model) below are **superseded**, not rewritten — identically to [ufls-module.md](ufls-module.md)'s own status update, which this document's §7.1 comparison table should now be read alongside. `UvlsSchemeVersion`'s lifecycle is now `Draft → Published → Superseded | Entered in Error` ([ADR-015](../adr/ADR-015-defence-scheme-version-lifecycle-simplification.md)). Region-scoped stage threshold/order/delay fields are now owned by a separate, reusable, region-scoping-capable Stage Setting Set ([stage-setting-set-architecture.md](stage-setting-set-architecture.md), [ADR-016](../adr/ADR-016-stage-setting-set-as-reusable-versioned-entity.md)), not embedded directly on `UvlsStage`. Pocket assignment now resolves via `CircuitTerminal` selection and `traverse` ([boundary-pocket-architecture.md](boundary-pocket-architecture.md), [ADR-017](../adr/ADR-017-boundary-pocket-assignment-architecture.md)) — the engineering judgement that pocket assignment remains rare for UVLS specifically (§7.4) is unaffected. Cross-Scheme Compliance's gate is now [findings-and-publication-governance-architecture.md](findings-and-publication-governance-architecture.md). See [uvls-engineering-philosophy.md](uvls-engineering-philosophy.md) for what remains genuinely UVLS-specific under the new model. §1–§6, §9 (minus lifecycle-specific rules), §10, §12–§18 remain authoritative and are **not** superseded.
