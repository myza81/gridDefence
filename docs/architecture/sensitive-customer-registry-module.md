...
**Status: Proposed — architecture discovery, not yet approved.** This document, its companion [EDR-008](../engineering/edr/EDR-008-sensitive-customer-registry-scope.md), and [ADR-012](../adr/ADR-012-sensitive-customer-registry-architecture.md) together define the engineering philosophy, ownership boundaries, and integration contracts for this module. No implementation (database schema, SQLAlchemy models, migrations, APIs, frontend, or tests) may begin until all three are reviewed and approved by the Project Owner (CLAUDE.md §24). Sections 11 and 12 below are deliberately conceptual — engineering entities and required capabilities, not implementation schema or REST routes — per this task's own scope.
