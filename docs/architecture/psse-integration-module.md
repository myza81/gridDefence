...
**Status update (Foundation Hardening Sprint B — Architecture Boundary Review, PARTIALLY CORRECTED).** A compliance audit found the implementation had drifted from the paragraph above for the Substation Registry line specifically: `PsseIntegrationService` imported `Substation` (Substation Registry's own persistence model) directly and queried it via this module's own repository/db session, for both bus-to-substation mnemonic matching (`_match_substation_for_bus`) and mnemonic display enrichment (`_map_entry_summary`). This is now corrected — both call `SubstationService` (`find_by_mnemonic`, `get_substation`) directly, per the documented intent; `PsseIntegrationRepository.find_substation_by_mnemonic_ci` (the now-unused direct-model-query method) was removed. `PsseIntegrationRepository.list_substations_by_ids` was deliberately **not** changed — it is a bulk, display-only read (Correlated Operational Model mnemonic display across many already-matched Buses in one query), which remains within CLAUDE.md A1's query-optimisation exception; routing it through `SubstationService` one substation at a time would reintroduce the N+1 query pattern that exception exists to avoid.

**Remaining gap, not corrected in this sprint (documented, not hidden):** the Equipment Registry line above — `CircuitTerminal`/`Circuit`/`SubstationVoltageYard` reads feeding `EquipmentTopologyMap` matching (`_run_matching`, `_terminal_substation_id`, `_map_entry_summary`, `get_circuit_correlation`) — still goes through this module's own repository importing Equipment Registry's models directly, not through `EquipmentRegistryService`. This is a genuine, confirmed gap against this section's own documented intent. It was not corrected here because a faithful fix requires new, precisely-scoped bulk read methods on Equipment Registry's service layer that exactly reproduce today's `ENTERED_IN_ERROR`-exclusion semantics across potentially thousands of terminals per import — a larger, higher-risk change to this module's correctness-critical matching pipeline than "smallest change preserving behaviour" (this sprint's own governing instruction) permits taking on incidentally. Recommended as its own, dedicated, carefully-tested follow-up — see [codex-foundation-readiness-audit-brief.md](codex-foundation-readiness-audit-brief.md)-style scoping before attempting it.
