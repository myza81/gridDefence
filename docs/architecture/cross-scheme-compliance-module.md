...
> **Status update (Engineering Scheme Architecture Pack, six-workshop conclusions).** This module's **detection logic** (§ Rule 1 Algorithm Concept, § Rule 2 Algorithm Concept) remains fully valid and is reused, unchanged, as two of the detectors [findings-and-publication-governance-architecture.md](findings-and-publication-governance-architecture.md) composes. This module's **governance mechanism** — `ComplianceRuleConfig`/`ComplianceCheckRun`/`ComplianceFinding` as a dedicated gating entity family, and the three-checkpoint (Submit for Review/Approval/Activation) validation timing in §8–§9 — is **superseded** by [findings-and-publication-governance-architecture.md](findings-and-publication-governance-architecture.md)'s unified findings/publication-treatment model, gated once, at the single Publish checkpoint [ADR-015](../adr/ADR-015-defence-scheme-version-lifecycle-simplification.md) defines. See [ADR-018](../adr/ADR-018-findings-severity-and-publication-governance-separation.md) §9 for the full reconciliation. §1–§7 (module overview, purpose, ownership, domain model), §10–§17 (validation rules not tied to the superseded checkpoint model, database design as a reference for what a detector's own supporting data might look like, security, testing) remain a useful reference and are **not** rewritten by this update.
