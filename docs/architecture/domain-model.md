...
- **Sensitive Customer Registry** (not yet designed — pending its own EDR and ADR; architecture-discovery has not begun) — policy-exclusion classification for candidate loads, structurally expected to follow the same Engineering Registry pattern as Critical Infrastructure and the Automatic Load Shedding Functionality Registry. Per the accepted architecture review following ALSF's completion, this is the recommended next architecture-discovery phase — see [implementation-plan.md](implementation-plan.md)'s roadmap notes.
