...
> **Forward decision — Transformer Engineering Endpoint model ([EDR-011](../engineering/edr/EDR-011-transformer-engineering-interface-model.md), [ADR-028](../adr/ADR-028-transformer-engineering-endpoint-architecture.md)).** The Phase 3.5 model below requires **both** `TransformerTerminal` rows to resolve to a registered `SubstationVoltageYard` at the transformer's substation. That correctly expresses Inter-bus and Load transformers but not Generation Step-Up (GSU), Station Service (SST), or Customer Supply (CST) transformers, whose non-grid winding interfaces a system that is not a registered Voltage Yard (EDR-011). ADR-028 decides that a `TransformerTerminal` will instead carry a discriminated **Engineering Endpoint**: a **Registered** endpoint keeps the existing `voltage_yard_id` FK (unchanged), while a **Typed** endpoint (Distribution System / Generation Collector System / Station Auxiliary System / Customer Installation) carries its own nominal voltage level and optional label, with a mutual-exclusion `CHECK` (the ALSF polymorphic-target shape). Transformer classification becomes an engineer-asserted curated reference value (never derived from the endpoint); the same-substation and uniqueness rules generalize from a "yard pair" to an "endpoint pair"; the voltage-order rule is preserved via the endpoint's nominal voltage; and existing terminals migrate to `endpoint_kind = TRANSMISSION_VOLTAGE_YARD` with `voltage_yard_id` untouched (fully backward-compatible). **This is a ratified architecture decision, not yet implemented** — the Business Rules, Database Design, and API text below still describe the current two-Voltage-Yard model and are left as-is per this document's immutable-history practice; read them together with ADR-028 for the decided target model. The subsequent implementation phase will update this section in place.
...
- `GET /api/v1/transformer-terminals` — every Transformer Terminal across every substation, with full composed identity (substation, voltage level, transformer short name, side), unpaginated (Phase 3.7 UAT refinement, added to support the Sensitive Customer Registry's cross-module multi-select picker). Exposed as its own top-level resource — `/transformer-terminals`, not `/transformers/{id}/terminals` — mirroring `VoltageYard`'s own established `/voltage-yards` resource (this document's ADR-008 addendum; also referenced at §"Corrected records..." and the UAT-correction list above): a caller here needs to browse or filter Transformer Terminals *across* their parent, not only within one already-selected Transformer's context, exactly the same shape of need `VoltageYard` already has relative to `Substation`. This is different in kind from `CircuitTerminal` (`GET /circuits/{id}/terminals`), which stays path-nested because a Circuit Terminal is only ever meaningful, added, or edited in the context of one specific Circuit already being viewed. Authentication required, no additional permission gate — same read-access pattern as every other list endpoint in this module (mirroring `equipment_registry.read`'s own absence, per this module's read-endpoints-require-only-authentication convention above).
  ...
  **Status update (Foundation Hardening Sprint A.1).** The "different in kind... stays path-nested" reasoning above governed *write* context (adding/editing a terminal only makes sense within one already-selected Circuit) — it did not anticipate a purely read-only, cross-network *diagnostic browse* need. `GET /api/v1/circuit-terminals` now exists too, alongside the unchanged `/circuits/{id}/terminals`, mirroring `/transformer-terminals` exactly (full composed `CircuitTerminalIdentity`, unpaginated, authentication-only), added specifically for the Network Model's Boundary Pocket diagnostic evaluator (docs/architecture/boundary-pocket-architecture.md) to browse/select Circuit Terminal opening points across the whole network. This is an additional read access pattern, not a reversal of the write-context reasoning above.
...
9. **A Voltage Yard may be restored from `ENTERED_IN_ERROR` back to `ACTIVE` — the single, entity-specific exception to that status's terminality ([ADR-027](../adr/ADR-027-voltage-yard-restoration-from-entered-in-error.md)).** A switchyard's identity is `(substation_id, voltage_level_id)` and the uniqueness rule enforcing it is status-blind, so a corrected yard permanently occupies its identity slot and **no replacement can ever be created**; without a restoration path an accidental correction would leave GridDefence unable to represent a switchyard that physically exists. Voltage Yards therefore now have a **closed two-edge allow-list** — `ACTIVE → ENTERED_IN_ERROR` and `ENTERED_IN_ERROR → ACTIVE`, every other transition rejected with `InvalidVoltageYardStatusTransitionError` — which is *stricter* than the previously unguarded status field. `change_reason` is mandatory in both directions; a same-status update is an idempotent no-op with no audit row (mirroring `SubstationService.change_status`); restoration **appends** a new audit row and never alters the original correction entry; and restoration is refused while the parent Substation is in a terminal lifecycle state (`operational_status.is_terminal`). Governance lives in the service layer, so `PATCH /voltage-yards/{id}` and the dedicated `POST /voltage-yards/{id}/restore` are equally governed and no client can bypass the allow-list. This exception is grounded solely in the Voltage Yard's non-reproducible identity and **does not change `ENTERED_IN_ERROR` semantics for any other entity** — it remains terminal for `Substation`, `Circuit`, and `Transformer`, each of which can be superseded by a new record under a different identity.
