...
### 0.1 Rich mode is the default; env is an override

Rich mode is the **out-of-the-box** experience: `clone → npm install → npm run dev → Rich map` (when the internet is reachable), with **no `.env` required**. The Standard style resolves as `VITE_MAP_STYLE_STANDARD → VITE_MAP_STYLE_URL → built-in governed default` ([`DEFAULT_STANDARD_STYLE_URL`](../../frontend/src/components/map/mapConfig.ts) = OpenFreeMap Liberty, OSM/ODbL, no key — adopted per [licensing-policy.md §6b](../engineering/licensing-policy.md)). Neutral mode is entered **only when the provider is genuinely unavailable** (offline, blocked, or the style fails to load). So *"Neutral map"* now indicates a real provider-availability problem, not missing configuration.

**Overriding the default** (a paid/domain-keyed provider, a self-hosted style, or the offline package) — set it in `frontend/.env` and **restart Vite** (env is read at startup):

```
# frontend/.env  (optional override — see frontend/.env.example)
...
Critical/high-availability or air-gapped deployments **should** override the community default with a governed paid provider or the self-hosted offline package. Provider governance (accept/reject, attribution, keys) is in [licensing-policy.md §6b](../engineering/licensing-policy.md) and [THIRD_PARTY_NOTICES.md §5b](../../THIRD_PARTY_NOTICES.md). `VITE_*` values are browser-exposed build-time config, not secrets — but do not commit real provider tokens.
...
Investigated per-layer against the live OpenFreeMap Liberty style (not by layer count). The style is rich (111 layers: roads, road names, railways, rivers, buildings incl. 3D, industrial/residential land use, forests/parks, admin boundaries, place names, POIs), and GridDefence renders **all** of it — no hidden layers, no filtering, no simplification on our side. The decisive finding:

- **The sparseness at low/mid zoom is OpenMapTiles vector-tile *data generalization*, not style decluttering.** Evidence: `road_minor`, `road_secondary_tertiary`, `waterway_river`, `landcover`, `park`, `boundary` carry **no style `minzoom` and no zoom filter** — they render whenever the data is present. The data is simply **not encoded in low-zoom tiles** (minor roads/waterways/buildings appear only ~z13–14; POIs ~z15). **No `setLayerZoomRange` or style edit can reveal data that is not in the tile.** This is a property of *every* OpenMapTiles vector provider — the fundamental difference from the MVP's OSM-Carto **raster** (full detail rendered server-side at every zoom).
- **What is style-gated (and safe to nudge):** only labels/buildings — place labels (`label_city` z3 … `label_village` z9), road-name labels (`highway-name-major` ~z12.2, `highway-name-minor` ~z15), `waterway_line_label` z10, `building` z13.
...
**Power infrastructure:** the OpenMapTiles schema has **no power source-layer**, so substations/transmission lines/towers **cannot** be shown by any style edit — and per §0.4 they would remain External Geographic Context, not engineering truth, so they **should not** be shown as GridDefence data regardless.

**Provider decision — keep Liberty (no migration).** Liberty is a rich, governed (OSM/ODbL, no key, self-hostable), MapLibre-native vector style and is sufficient long-term for an engineering basemap; its low-zoom generalization is intrinsic to *all* vector OSM providers. The only way to get the MVP's all-zoom raster density is a **raster** basemap (OSM-Carto), which for enterprise means a **keyed or self-hosted** raster provider — documented as an optional `VITE_MAP_STYLE_STANDARD` override, **not** a forced migration and **not** the public OSMF tile server (whose usage policy forbids such use).
