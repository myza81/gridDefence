...
## Addendum (2026-07-23): Voltage Yard Restoration from Entered in Error

**Superseding note for lifecycle only — the entity, its identity, and its uniqueness rule are unchanged by this addendum.** See [ADR-027](ADR-027-voltage-yard-restoration-from-entered-in-error.md) for the full decision.

The Deletion/Correction Policy gave `SubstationVoltageYard` an `operational_status_id` but never an accompanying transition allow-list, so any status could follow any other. ADR-027 closes that gap and, in doing so, records the one place where `ENTERED_IN_ERROR` is **not** terminal in GridDefence.

The reason is this ADR's own identity decision. A Voltage Yard is identified by `(substation_id, voltage_level_id)` and protected by `UniqueConstraint("substation_id", "voltage_level_id")`; the service-layer duplicate check backing it is **status-blind**. A yard corrected as Entered in Error therefore keeps occupying its identity slot forever, and no replacement can be created for that substation at that voltage level. Unlike a Substation (re-created under a different mnemonic) or a Circuit/Transformer (re-created under a different bay/transformer number), a Voltage Yard has no alternative identity to be re-created under — so terminality would permanently prevent GridDefence from representing a switchyard that physically exists.

ADR-027 therefore defines a closed two-edge allow-list for this entity — `ACTIVE → ENTERED_IN_ERROR` and `ENTERED_IN_ERROR → ACTIVE`, everything else rejected — with a mandatory `change_reason` in both directions, an append-only audit entry for the restoration, and a refusal to restore while the parent Substation is in a terminal state. This is stricter than the previously unguarded field, changes neither the identity rule nor the uniqueness constraint, and does not alter `ENTERED_IN_ERROR` semantics for any other entity.
