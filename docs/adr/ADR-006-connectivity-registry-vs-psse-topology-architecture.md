...
6. [domain-model.md](../architecture/domain-model.md) §4 ("Network Model Domain") still describes its future scope in terms of an owned "connectivity record — e.g. a transmission line entity referencing a substation at each end," predating [ADR-003](ADR-003-psse-topology-and-load-snapshot-separation.md)'s later decision that Network Model owns no topology data of its own. This ADR's conclusion (§13) is consistent with ADR-003's later, more specific decision, not with domain-model.md §4's earlier phrasing — domain-model.md should be updated to reflect this in a future documentation pass, but that edit is not made by this ADR itself (ADRs record decisions; domain-model.md's own maintainers should reconcile the wording). **Resolved by the Addendum below** (Architecture Synchronization Sprint) — domain-model.md §4 has since been updated.

---

## Addendum (2026-07-07): Operational Snapshot Pivot — Clarification, Not a Reopening

**This ADR remains valid, Accepted, and is not superseded by this addendum.** [EDR-007](../engineering/edr/EDR-007-phase-7-operational-identity-mapping.md) and the new `docs/architecture/operational-snapshot-architecture.md` establish that **Operational Snapshot is authoritative for current electrical topology and operational state; Engineering Registries remain authoritative for curated engineering identity and metadata.** This is not a new decision in tension with this ADR — it is the same identity/topology separation this ADR's Option D already drew, restated at the level of "Operational Snapshot" as a general architectural concept rather than only PSS/E Integration's own entities.

- **Equipment Registry / Line Connectivity Registry** owns engineering identity and metadata (§6, §7 above: bay numbers, breaker numbers, line type, commissioning dates, interconnector flags, alias/rename history) — unchanged by this addendum.
- **PSS®E / Operational Snapshot** owns electrical topology and current connectivity (§6, §7 above: `TopologyVersion` and its structural children) — unchanged by this addendum, now generalized under the Operational Snapshot architecture.
- **The Operational Snapshot pivot strengthens this ADR's identity/topology separation; it does not contradict it.** Where this addendum adds something new is scope, not substance: it clarifies that Phase 5's own static, registry-derived connectivity model (`network_model`, built after this ADR was written — see [network-model-module.md](../architecture/network-model-module.md) §19) is, itself, a curated-metadata/correlation view under this same Option D split, not a third source of connectivity truth this ADR would need to arbitrate. `EquipmentTopologyMap` (§8, §9 above) remains the correlation mechanism between the two, exactly as this ADR specified.
- **No change to §5 (Recommended Architecture), §6 (Source of Truth Responsibilities), §7 (Data Ownership), §8 (Network Change Management), §9 (Reconciliation Strategy), or §13 (Decision).** Read this addendum as reinforcing those sections, not amending them.
