...
| **Line Bay** | `CircuitTerminal` (Equipment Registry) | One substation's own terminal of a `Circuit`. The object PSS®E's `EquipmentTopologyMap` already correlates against, and the object each Automatic Load Shedding Functionality Registry (Relay Registry) entry attaches to. |
...
| **Grid Defence Capability** / **Relay Capability** (automatic UFLS/UVLS scope) | **Automatic Load Shedding Functionality Registry** (complete — [ADR-011](../adr/ADR-011-automatic-load-shedding-functionality-registry.md), `docs/architecture/automatic-load-shedding-functionality-registry-module.md`) | Attaches to a `CircuitTerminal` or `TransformerTerminal` — i.e., to a Bay — exactly as `EquipmentTopologyMap` already does for PSS®E correlation. **"Relay Registry" is retired as a working name** ([ADR-011](../adr/ADR-011-automatic-load-shedding-functionality-registry.md)): the concept previously discussed under that name ([EDR-003](edr/EDR-003-relay-registry-scope.md)) is realized as this dedicated registry, not as a section inside Equipment Registry — a change from the direction `equipment-registry-module.md` §7.8 (superseded) originally sketched. Scoped to UFLS/UVLS only; EMLS has no automatic-functionality prerequisite and is never referenced by this registry. UFLS/UVLS themselves are not yet built — their own Selection of Shedding Actions design must call `is_ufls_capable`/`is_uvls_capable` and the candidate-search interfaces (module document §13) when that work begins. |
...
**Implementation phrasing (equivalent):** "An Automatic Load Shedding Functionality Registry entry, keyed to a `transformer_terminal_id`, is checked before a scheme module accepts that `TransformerTerminal` as a candidate for a shedding assignment."
