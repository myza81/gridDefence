...
**Definition.** Engineering Knowledge is the slow-changing, engineer-curated information describing the physical engineering environment — substation and equipment identity, line/circuit and transformer engineering attributes, relay capability, and sensitive customer designations.
...
**Engineering significance.** Engineering Knowledge is the authoritative reference layer for identity and metadata — when it disagrees with an inference drawn from Operational Context (for example, PSS®E topology implying a circuit's far end has changed), the disagreement is a **Validation** finding requiring engineering review, never something Operational Context is allowed to silently overwrite. Engineering Knowledge is **not** the authoritative reference for *current* electrical topology or *current* operational connectivity — that is Operational Context's role (Layer 2). Neither layer overwrites the other; GridDefence correlates them.

**Relationship to other concepts.** Engineering Knowledge is Layer 1 of the three **Engineering Information Layers** (01-engineering-philosophy.md §6): the Substation Registry, Equipment Registry, Line Connectivity Registry, **Relay Registry**, and Sensitive Customer Registry. It is referenced by, but never derived from, **Operational Context** or **Engineering Decisions**. **Line Connectivity Registry** specifically is a curated metadata registry for line/circuit identity and engineering attributes (bay numbers, breaker numbers, line type, interconnector flags, commissioning dates) — it is not the authoritative source of current electrical topology, which **PSS/E Network Topology** (Operational Context) provides. The same distinction applies to the transformer-asset portion of Equipment Registry: it is a curated metadata registry for transformer identity and engineering attributes, not the authoritative source of current operational transformer representation. See [EDR-007](edr/EDR-007-phase-7-operational-identity-mapping.md) and `docs/architecture/operational-snapshot-architecture.md`.
