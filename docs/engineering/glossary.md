...
**Engineering Endpoint** — The electrical-system domain a transformer terminal (winding side) interfaces with, in the Layer 1 engineering model. A **Registered Engineering Endpoint** resolves to a GridDefence-owned registry object (today, exactly one kind: a Transmission Voltage Yard); a **Typed Engineering Endpoint** is a named engineering system GridDefence recognises but does not yet register as a first-class object — a Distribution System, Generation Collector System, Station Auxiliary System, or Customer Installation (a genuine engineering system outside current registry scope, not an "external" object, and a candidate to become Registered if a corresponding registry is later created). See [EDR-011](edr/EDR-011-transformer-engineering-interface-model.md).
...
**Transformer Engineering Classification** — The engineer-asserted statement of which electrical-system domains a transformer's windings interface with — a Layer 1 engineering-identity attribute, never defined by voltage ratio. The ratified classifications are: **Inter-bus Transformer** (Transmission Grid ⇄ Transmission Grid), **Load Transformer** (Transmission Grid ⇄ Distribution System), **Generation Step-Up Transformer (GSU)** (Generation Collector System ⇄ Transmission Grid), **Station Service Transformer (SST)** (Transmission Grid ⇄ Station Auxiliary System), and **Customer Supply Transformer (CST)** (Transmission Grid ⇄ Customer Installation). The same voltage ratio (e.g. 132/33 kV) may be a Load, SST, or CST depending on the non-grid domain interfaced. Classification is independent of GridDefence application usage — it never implies defence-scheme eligibility, which is expressed only through capability and assignment. See [EDR-011](edr/EDR-011-transformer-engineering-interface-model.md) and, for the operational-side taxonomy it corresponds to, [EDR-007](edr/EDR-007-phase-7-operational-identity-mapping.md).
