...
*Of these, Substation Registry, Equipment Registry, and Relay Registry (implemented as the Automatic Load Shedding Functionality Registry, [ADR-011](../adr/ADR-011-automatic-load-shedding-functionality-registry.md)) are built; Sensitive Customer Registry is the recommended next architecture-discovery phase. This document's own scope is categorical, not a schedule (§ above) — see [implementation-plan.md](../architecture/implementation-plan.md) for build status and sequencing.*
